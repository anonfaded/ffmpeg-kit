...
APP_LDFLAGS := -Wl,--hash-style=both,--build-id=none
EOF
}
...
get_ldflags() {
  ...
  echo "${ARCH_FLAGS} ${OPTIMIZATION_FLAGS} ${COMMON_LINKED_LIBS} -Wl,--hash-style=both,--build-id=none -Wl,--exclude-libs,libgcc.a -Wl,--exclude-libs,libunwind.a"
}
